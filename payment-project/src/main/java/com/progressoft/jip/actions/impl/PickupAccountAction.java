...
public class PickupAccountAction extends AbstractPaymentAction<StringContainer>
		implements PaymentNewAction<StringContainer> {
	...
	public void init() {
		setAction(dynamicFormActionBuilder.setInterfaceType(StringContainer.class).setSubmitAction(this)
				.setForm(new FormImpl(PICKUP_ACCOUNT_FORM_DESCRIPTION).addField(
						new StringField().setDescription(ENTER_ACCOUNT_NUMBER).setName(StringContainer.VALUE_NAME)))
				.build());
	}

	@Override
	public void process(PaymentMenuContext menuContext, StringContainer stringContainer) {
